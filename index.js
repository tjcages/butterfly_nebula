const url =
    "https://pbs.twimg.com/media/FaixRJQUYAAgFzi?format=jpg&name=900x900",
  extrusion = {
    // how far each particle should be extruded based on its HSV color
    h: 0.25,
    s: -0.1,
    v: 0,
  },
  mouseExtrusion = 2; // extrusion multiplier on mouse over
...
let texture = loader.load(url, (texture) => {
  ...
  new TWEEN.Tween(camera.position)
    .to({ z: Math.max(imageWidth, imageHeight) * 0.7 }, 5000)
...
function render(time) {
  ...
  if (mesh) {
    let xRotation =
      Math.PI + Math.cos(currentTime * 0.25 + 20) * (half_PI * 0.25);
    let yRotation =
      Math.PI + Math.sin(currentTime * 0.15 - 1) * (half_PI * 0.25);
    ...
    mesh.material.uniforms.hsv.value.set(
      currentExtrusion.h,
      currentExtrusion.s,
      currentExtrusion.v
    );
